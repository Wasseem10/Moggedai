...
async function generateSkipReply(habit: HabitRow, coachStyle: string): Promise<string> {
  ...
  const styleGuide: Record<string, string> = {
    ...
    savage: 'one sentence. make it sting. say the thing they do not want to hear.',
...
export async function POST(req: NextRequest) {
  try {
    const event = await req.json() as TelnyxWebhook
    ...
    if (!from) return NextResponse.json({ success: true, ignored: true })

    await ensureSchema()
    const db = getDb()
    const digits = from.replace(/\D/g, '')
    const e164 = digits.length === 11 ? `+${digits}` : from

    const { rows: userRows } = await db.query(
      `SELECT u.id, p.coach_style, p.biggest_distraction
       FROM users u
       LEFT JOIN profiles p ON p.user_id = u.id
       WHERE u.phone = $1 OR u.phone = $2 OR u.phone = $3 LIMIT 1`,
      [digits, from, e164]
    )
    ...
    if (/^(stop|unsubscribe|cancel|quit)$/i.test(body)) {
      ...
      return reply(from, "you've been unsubscribed. text START anytime to get back on track.")
    }
    ...
    if (/^start$/i.test(body)) {
      ...
      return reply(from, "you're back. let's get it.")
    }

    if (!userRows.length) {
      return reply(from, "i don't have you in the system. sign up at moggedai.com")
    }
    ...
    if (hasMedia) {
      ...
      return reply(from, await generatePhotoReply(activeHabit, coachStyle, streak))
    }

    if (/^habits?$/i.test(body)) {
      if (!habits.length) return reply(from, "no habits set up yet. go to moggedai.com to add some.")
      const list = habits.map((h: HabitRow) => `${h.emoji} ${h.name}`.trim()).join('\n')
      return reply(from, `your goals:\n${list}`)
    }
    ...
    if (/^streak$/i.test(body)) {
      ...
      for (const h of habits) {
        ...
        const streak = getStreak(comps)
        lines.push(`${h.emoji} ${h.name}: ${streak} day streak`.trim())
      }
      return reply(from, lines.join('\n') || "no streaks yet. get moving.")
    }
    ...
    if (/^(done|finished|completed|did it|just did|i did it)/i.test(body)) {
      ...
      const streak = getStreak(comps)
      return reply(from, await generateDoneReply(activeHabit, coachStyle, streak))
    }

    if (/^skip$/i.test(body)) {
      return reply(from, await generateSkipReply(activeHabit, coachStyle))
    }

    return reply(from, await generateCoachReply(body, activeHabit, coachStyle, recentCheckIns))
  } catch (err) {
